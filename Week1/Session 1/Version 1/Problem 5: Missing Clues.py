...
def find_missing_clues(clues, lower, upper):
    ...
    for clue in sorted(clues) + [upper + 1]:
        if clue - prev > 1:
...
if __name__ == "__main__":
    ...
    assert find_missing_clues([], 0, 99) == [[0, 99]]
    assert find_missing_clues([0, 1, 2], 0, 2) == []
    assert find_missing_clues([1, 2], 0, 2) == [[0, 0]]
    assert find_missing_clues([0, 1], 0, 2) == [[2, 2]]
    assert find_missing_clues([0, 1, 3, 50, 75], 0, 99) == [[2, 2], [4, 49], [51, 74], [76, 99]]
